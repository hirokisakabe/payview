...
test("正常系: CSVファイルがパースされる", async () => {
  ...
  expect(result.csvData).toEqual(mockCsvData);
});
...
test("正常系: 複数行のCSVがパースされる", async () => {
  ...
  expect(result.csvData).toHaveLength(2);
});

test("異常系: CSVが空の場合、ConvertFileToCsvInvalidCsvErrorをthrowする", async () => {
  vi.mocked(Encoding.convert).mockReturnValue([]);
...
test("異常系: パース中にエラーが発生した場合、ConvertFileToCsvUnknownErrorをthrowする", async () => {
  vi.mocked(Encoding.convert).mockReturnValue([]);
