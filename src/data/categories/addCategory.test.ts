...
import { addCategory, AddCategoryError } from "./addCategory";
...
test("異常系: DB操作でエラーが発生した場合", async () => {
  ...
  const result = await addCategory({ name: "エラー" });

  expect(result.isErr()).toBe(true);
  expect(result._unsafeUnwrapErr()).toBeInstanceOf(AddCategoryError);
